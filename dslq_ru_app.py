...
GH_OPTIONS_ALL: List[Tuple[int, str]] = [
    ...
    (4, "Есть проблемы с желудком/кишечником (запор, диарея, рвота и т.п.)"),
...
def screen_general_health() -> None:
    ...
    st.markdown(
        '<p class="dslq-section-label">Замечали ли вы у вашей собаки в последнее время следующие изменения, связанные со здоровьем?</p>'
...
def screen_sharing() -> None:
    ...
    st.markdown(
        '<div class="dslq-card">'
        f"<b>{c('consent_optional_title', 'Необязательное согласие на использование данных в исследовании')}</b><br><br>"
        + c(
